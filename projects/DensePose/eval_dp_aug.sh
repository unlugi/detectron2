...
CUDA_VISIBLE_DEVICES=2 python3 train_net.py --config-file configs/sketch/densepose_rcnn_R_50_FPN_s1x_legacy.yaml \
                                            --eval-only \
                                            MODEL.WEIGHTS "$OUTPUT_FOLDER/output3/model_final.pth" \
                                            OUTPUT_DIR "$OUTPUT_FOLDER/output3" \
...
CUDA_VISIBLE_DEVICES=2 python3 train_net.py --config-file configs/sketch/densepose_rcnn_R_50_FPN_s1x_legacy.yaml \
                                            --eval-only \
                                            MODEL.WEIGHTS "$OUTPUT_FOLDER/output4/model_final.pth" \
                                            OUTPUT_DIR "$OUTPUT_FOLDER/output4" \
                                            MODEL.KEYPOINT_ON True \
# Give it a little time
sleep 10
...
CUDA_VISIBLE_DEVICES=2 python3 train_net.py --config-file configs/sketch/densepose_rcnn_R_50_FPN_s1x_legacy.yaml \
					    --eval-only \
					    MODEL.WEIGHTS "$OUTPUT_FOLDER/output5/model_final.pth" \
					    OUTPUT_DIR "OUTPUT_FOLDER/output5" \
                                            MODEL.KEYPOINT_ON True \
